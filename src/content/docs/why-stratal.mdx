...
That's what Stratal is. A modular framework — inspired by the architecture of NestJS and the conventions of Laravel — that builds DX and structure on top of a great platform. If you just want to start building, head to the [installation guide](/getting-started/installation/).
...
If this looks familiar, it should — Stratal's module system draws directly from NestJS, adapted for the Cloudflare Workers runtime.
...
`Stratal` implements `fetch()`, `queue()`, and `scheduled()` for you. Define your module tree, and the framework handles the rest.
