...
<Aside type="tip">
When using `npx inertia dev`, a Vite config is auto-generated. You only need a custom `vite.config.ts` if you need additional Vite plugins or custom configuration.
</Aside>
...
## Using with `npx inertia dev`

The recommended way to develop Inertia applications is with the `@stratal/inertia` CLI:
...
If you need additional Vite plugins or custom configuration beyond what `npx inertia dev` provides, create a `vite.config.ts` in your project root:
...
When a `vite.config.ts` is present, both `npx inertia dev` and `npx inertia build` will use it instead of the auto-generated configuration.
