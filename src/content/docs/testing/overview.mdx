...
<Aside type="tip">
  `stratalTest()` wraps `cloudflareTest` from `@cloudflare/vitest-pool-workers` and applies Stratal-specific defaults: a tslib alias for decorator helpers, ZenStack language mocks, a nodemailer mock, and SSR externals configuration. You do not need to configure these manually.
</Aside>
