...
<Aside type="note">
  `@stratal/testing` bundles `@cloudflare/vitest-pool-workers` (0.13.0+) — you do not need to install it separately. `vitest` (>= 4) is a peer dependency that you must install yourself.
</Aside>
...
<Aside type="tip">
  `stratalTest()` wraps `cloudflareTest` from `@cloudflare/vitest-pool-workers` and applies Stratal-specific defaults: a tslib alias for tsyringe, ZenStack language mocks, a nodemailer mock, and SSR externals configuration. You do not need to configure these manually.
</Aside>
...
Create a `vitest.setup.ts` that imports `reflect-metadata` (required in tests since they don't go through the `Stratal` entry point, which handles this import automatically in production) and configures shared base modules:
