...
```bash
yarn add -D @stratal/testing @cloudflare/workers-types vitest
```

<Aside type="note">
  `@stratal/testing` bundles `@cloudflare/vitest-pool-workers` (0.13.0+), so you do not need to install it separately. `vitest` (>= 4) is a peer dependency that you must install yourself.
</Aside>
...
Create a `vitest.config.ts` at the root of your project using the `stratalTest()` plugin:
...
<Aside type="tip">
  `stratalTest()` wraps `cloudflareTest` from `@cloudflare/vitest-pool-workers` and applies Stratal-specific defaults: a tslib alias for decorator helpers, ZenStack language mocks, and SSR externals configuration. You do not need to configure these manually. Email sending is handled separately at runtime: `compile()` installs an in-memory `TestEmailProvider` so tests never open a real SMTP connection.
</Aside>
...
Create a `vitest.setup.ts` that configures shared base modules:
...
Your project needs a `wrangler.jsonc` (or `wrangler.toml`) with the `nodejs_compat` compatibility flag enabled. This is required for Node.js APIs used by the framework:
...
<Aside type="note">
  `.compile()` initializes routing eagerly and installs safe test defaults: in-memory storage, an in-memory email provider, and a no-op rate limiter so suites never trip production limits. See [Mocks and Fakes](/testing/mocks-and-fakes/) for how to assert on sent email and how to opt back into real rate limiting, and [Testing Module](/testing/testing-module/) for what `compile()` does step by step.
</Aside>
