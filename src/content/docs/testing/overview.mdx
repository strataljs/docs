...
Create a `vitest.setup.ts` that imports `reflect-metadata` (required in tests since they don't go through `StratalWorker`, which handles this automatically in production) and configures shared base modules:
