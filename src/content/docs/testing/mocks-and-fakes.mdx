...
`@stratal/testing` provides several tools for isolating your code from external dependencies: deep mocks with `createMock`, an in-memory `FakeStorageService`, fetch interception with `MockFetch` (built on [MSW](https://mswjs.io/)), and a nodemailer mock for email testing.
