...
`compile()` builds and fully initializes the application before resolving:

<Steps>

1. Builds a root module from your config, prepending any [base modules](#shared-base-modules).

2. Constructs the `Application` and runs `initialize()`.

3. Installs test defaults: a `FakeStorageService` for storage, a `FakeFeatureFlagService` for feature flags, a `NoopRateLimiterStore` so suites never trip rate limits, and an in-memory `TestEmailProvider` so no SMTP connections open. See [Mocks and Fakes](/testing/mocks-and-fakes/) for the rate limiter and email defaults.

4. Applies your provider overrides, so they replace both module-registered providers and the test defaults.

5. Initializes routing eagerly, so the router and its services are built during `compile()` rather than on the first request.

</Steps>

Because routing is initialized eagerly, configuration errors surface as a `compile()` rejection instead of on the first HTTP request. For example, mixing `@Route()` with HTTP method decorators on the same handler fails the `await ...compile()` call, so the misconfiguration is caught at setup with a clear stack rather than deep inside a request.

<Aside type="caution">
  If `compile()` fails for any reason, it tears down the partially built application (shutting down module-held resources like database pools and timers) before rejecting. You do not need a `try/finally` around `compile()` to clean up a failed build, but you still call `module.close()` in `afterAll` for modules that compiled successfully.
</Aside>
...
| `sentEmails` | `ResolvedEmailMessage[]` | Messages recorded by the default `TestEmailProvider`, in send order |
| `featureFlags` | `FakeFeatureFlagService` | Fake feature-flag service for toggling flags in tests |
