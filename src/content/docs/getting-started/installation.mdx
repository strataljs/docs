...
Stratal can also expose your API routes as [Model Context Protocol (MCP)](https://modelcontextprotocol.io/) tools, letting AI agents discover and call your endpoints directly. Since Stratal generates an OpenAPI spec from your routes out of the box, the MCP server works with no additional setup.
...
See [AI Integration](/getting-started/ai/) for client configuration, filtering options, and remote dispatch.
...
<Tabs syncKey="pkg">
  ...
  <TabItem label="yarn">
    ```bash
    yarn add stratal reflect-metadata
    ```
  </TabItem>
  <TabItem label="pnpm">
    ```bash
    pnpm add stratal reflect-metadata
    ```
  </TabItem>
</Tabs>
