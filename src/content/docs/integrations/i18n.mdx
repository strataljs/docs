...
To support additional locales, call `I18nModule.forRoot()` in your root module. This method accepts locale configuration only — it does not accept messages.
...
@Module({
  imports: [I18nModule.forRoot({ defaultLocale: 'en', fallbackLocale: 'en', locales: ['en', 'fr'] })],
})
...
Use `I18nModule.registerMessages()` to add translations. This is separate from `forRoot()` so that any module — not just the root module — can contribute its own messages.
...
@Module({
  imports: [I18nModule.registerMessages({ en, fr })],
})
export class AppModule {}
```

You can call `registerMessages()` as many times as needed, from any module in your application. Each call deep-merges the provided messages with the existing set. If two calls define the same leaf key, the later call wins.

```typescript
// Root module registers app-wide messages
@Module({
  imports: [
...
// PaymentsModule registers its own messages
@Module({
  imports: [
    I18nModule.registerMessages({
      en: { payments: { receiptSent: 'Receipt sent to {email}' } },
      fr: { payments: { receiptSent: 'Reçu envoyé à {email}' } },
    }),
  ],
})
...
This makes it easy for packages and feature modules to ship their own translations without requiring the root module to know about them.
...
All registered messages are deep-merged with the built-in system messages. If you define a key that already exists in the system messages, your value takes precedence.
...
export class UserNotFoundError extends ApplicationError {
  constructor(userId: string) {
    super('users.errors.notFound', ERROR_CODES.USERS.NOT_FOUND, { userId })
  }
}
```
...
The module uses TypeScript module augmentation to provide autocomplete for message keys. Each module or feature owns one distinct top-level namespace on the `AppMessageNamespaces` registry:
...
<Aside type="note">
  Each module must own exactly **one** key on `AppMessageNamespaces`. Two modules augmenting the same key with different shapes will fail with TypeScript error **TS2717** ("Subsequent property declarations must have the same type"), because interface merging requires same-named properties across declarations to be structurally identical. This is the guardrail that keeps namespace ownership unambiguous — pick a distinct top-level key for every module (`users`, `billing`, `tenancy`, …) and nest your errors/validation/etc. underneath it.
</Aside>
