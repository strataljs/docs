---
title: Cron Jobs
description: Schedule recurring tasks with Cloudflare Cron Triggers using the CronJob interface, error handling, and the Stratal scheduled handler.
---
...
The `CronJob` interface declares two members:
...
The `schedule` is declared as a **static** property on the class, not as part of the `CronJob` interface. Stratal reads it off the class at registration time, before any instance is created, so it must be `static`:

```typescript
static schedule = '0 2 * * *' // Cron expression; must match a trigger in wrangler.jsonc
```

<Aside type="caution">
  A job class without a `static schedule` property is skipped at boot: Stratal logs a warning naming the class and never registers it. If a job silently never runs, confirm `schedule` is declared `static` and that its expression exactly matches a trigger in `wrangler.jsonc`.
</Aside>
...
export class RefreshCacheJob implements CronJob {
  static schedule = '0 * * * *' // Same schedule
...
Stratal ships the `FailedJobCleanupJob` cron, which deletes aged queue failed jobs. It is opt-in: add it to a module's `jobs` array and a matching trigger to `wrangler.jsonc`. See [Queues, automatic cleanup](/integrations/queues/#automatic-cleanup-opt-in-cron) for configuration and retention details.
...
Use the `schedule:list` [Quarry CLI](/core-concepts/quarry-cli/) command to see all registered cron jobs in your application:
