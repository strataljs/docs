...
<Aside type="caution">
  Do not use the tiered cache for read-modify-write counters that need cross-edge freshness, such as rate limiting. The L1 only observes writes made through its own isolate, so an isolate reads its own value until the entry expires. Concurrent increments from other isolates are missed and overwritten. Use plain `CacheService` (KV) or a Durable Object store for those.
</Aside>
...
| --- | --- | --- |
| `CacheError` | `get`, `getWithMetadata`, `put`, `delete`, `list` | A KV operation failed |
