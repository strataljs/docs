...
Use module augmentation to make `StratalEnv` extend the generated `Env` interface:
...
declare module 'stratal' {
  interface StratalEnv extends Env {}
}
```
...
/** Extract only the queue binding keys (those ending in _QUEUE) from StratalEnv */
type QueueBindingKeys = {
  [K in keyof StratalEnv]: K extends `${string}_QUEUE` ? K : never
}[keyof StratalEnv]

/** Convert a binding name like NOTIFICATIONS_QUEUE to kebab-case queue name like notifications */
type BindingToQueueName<T extends string> =
  T extends `${infer Name}_QUEUE`
    ? Lowercase<Name> extends `${infer A}_${infer B}`
      ? `${Lowercase<A>}-${Lowercase<B>}`
      : Lowercase<Name>
    : never

/** Derive all queue names from StratalEnv bindings */
type DerivedQueueNames = BindingToQueueName<QueueBindingKeys & string>
...
This derives queue names from your `StratalEnv` bindings automatically. A binding named `NOTIFICATIONS_QUEUE` produces the queue name `notifications`. A binding named `ORDER_EVENTS_QUEUE` produces `order-events`.

The queue provider performs the reverse conversion at runtime: it takes a queue name like `order-events`, converts it to `ORDER_EVENTS_QUEUE` via `.toUpperCase().replace(/-/g, '_')`, and looks up the binding in the environment.

<Aside type="tip">
  By deriving queue names from `StratalEnv`, you only need to maintain bindings in one place. When you add a new queue binding to your Wrangler config and re-run `npx wrangler types`, the queue name is automatically available.
</Aside>
...
When you add or remove a binding in your Wrangler configuration, re-run `npx wrangler types` to regenerate the `Env` interface. Your `StratalEnv` declaration picks up the changes automatically.
