...
<Aside type="caution">
  Make sure every type declaration file is imported before the `Stratal` instance is created. If a file is not imported, its augmented types will not be visible to the rest of the application.
</Aside>
...
```mermaid
graph TD
  A["Cloudflare Runtime"] -->|"passes env, ctx"| B["Stratal.fetch()"]
