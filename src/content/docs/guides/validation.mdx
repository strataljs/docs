...
When validation fails, Stratal throws a `SchemaValidationError`, which returns a `400 Bad Request`. The default response body contains the error message and a timestamp:
...
`SchemaValidationError` carries an `issues` array describing every field that failed. Each issue includes:
...
To include those issues in the response body, register a renderable for `SchemaValidationError` in your [exception handler](/guides/error-handling/):
...
export class AppExceptionHandler extends ExceptionHandler {
  register() {
    this.renderable(SchemaValidationError, (error) => {
...
<Aside type="note">
  In development, the default response also includes a `stack` trace for debugging. This is automatically stripped in production.
</Aside>
...
Every Zod schema you pass to `@Route` is automatically reflected in the generated OpenAPI specification. The `z` you import from `stratal/validation` extends Zod with an `.openapi()` method for richer API documentation.
...
For richer documentation, use the `.openapi()` method. It accepts an object with `example`, `description`, `deprecated`, and other OpenAPI schema properties:
...
The request locale is resolved by the i18n module's detection strategy, which defaults to the `locale` cookie. See [Internationalization](/integrations/i18n/) for the available strategies.

```
// Request resolved to the 'fr' locale
POST /api/users
{ "email": "invalid" }
