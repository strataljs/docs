...
For authentication-only checks (no scopes), only [AuthModule](/framework/auth/) is required. When using scopes for permission checks, both [AuthModule](/framework/auth/) and [RbacModule](/framework/rbac/) must be configured.
