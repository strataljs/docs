...
<Aside type="note" title="Not the only 403">
  A 403 from a guard is always an `InsufficientPermissionsError`: the user is authenticated but lacks the required permission. Better Auth itself can raise other 403 responses during auth flows, such as a `FreshSessionRequiredError` when a sensitive operation needs a recently authenticated session. Those are not guard errors. See the [auth error reference](/framework/auth/#auth-errors) for the full set.
</Aside>
