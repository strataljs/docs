...
`makeMany()` returns an array. Use `count()` to specify how many instances to create (defaults to `1` when not called):
...
- [Seeders](/framework/seeders/) for populating databases with factories.
