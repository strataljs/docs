...
Seeders are automatically discovered from the module tree. Any class that extends `Seeder` and is listed in a module's `providers` array will be found by the CLI. Bare class and `useClass` providers are scanned; value and factory providers are skipped.
