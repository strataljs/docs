...
| `deleteRolesForUser(userId)` | Remove all roles for a user |
...
| `getImplicitUsersForRole(role)` | Get all users with a role (direct + inherited) |
...
Policies are persisted in the database via a ZenStack adapter. The `defaultPolicies` and `roleHierarchy` you define in `RbacModule.forRoot()` are seeded automatically on initialization.

<Aside type="tip">
  If you modify policies programmatically (e.g., adding roles or permissions at runtime), call `clearCache()` on the `CasbinEnforcerService` to ensure the changes take effect.
</Aside>
