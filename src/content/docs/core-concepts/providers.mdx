...
A `ClassProvider` maps a token to a class. Use it when the token differs from the class:
...
The provider's scope comes from the scope decorator on the class itself (`@Singleton()`, `@Request()`, or `@Transient()`), so `PostgresUserRepository` carries its own scope.
