...
To scope middleware to a specific version, group the relevant controllers and set the version on the group with `version()`:
...
export class UsersModule implements RouteConfigurable {
  configureRoutes(router: Router): void {
    // Apply only to the v1 users controller
    router.group([UsersV1Controller], (v1) => {
      v1.version('1').middleware(AuthMiddleware)
    })
  }
}
```

Stratal resolves the version and prefix automatically, so `version('1')` matches `/v1/api/users` and its sub-paths.
...
Set on a `Router` scope or a `group()` to target versioned routes when configuring middleware:
