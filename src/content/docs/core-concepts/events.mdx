...
Use the `event:list` [Quarry CLI](/core-concepts/quarry-cli/) command to see all registered event listeners in your application:
...
| Column | Description |
| --- | --- |
| Event | The event pattern the handler listens for |
| Listener | The listener class name |
| Method | The handler method name |
| Priority | The handler's priority value |
| Blocking | Whether the handler blocks the emitter |

## Next steps

- [Database Events](/framework/database-events/) for events emitted automatically by database operations, including entity mutation events that carry full before and after entity snapshots.
