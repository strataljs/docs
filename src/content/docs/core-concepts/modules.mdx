...
### Synchronous configuration with withRoot()

Use `withRoot()` when you have static configuration values available at import time:

```typescript
import { Module, DynamicModule } from 'stratal'

@Module({})
export class CacheModule {
  static withRoot(options: { ttl: number }): DynamicModule {
    return {
      module: CacheModule,
      providers: [
...
```typescript
@Module({
  imports: [CacheModule.withRoot({ ttl: 3600 })],
})
export class AppModule {}
```

### Asynchronous configuration with withRootAsync()

Use `withRootAsync()` when the configuration depends on other services that need to be resolved first:

```typescript
import { Module, DynamicModule, AsyncModuleOptions } from 'stratal'

@Module({})
export class DatabaseModule {
  static withRootAsync<TOptions>(
