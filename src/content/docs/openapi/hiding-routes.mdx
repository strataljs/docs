...
The `hideFromDocs` property also works in [HTTP method decorator](/core-concepts/http-method-decorators/) config:
...
<Aside type="note">
  There is no need to set `hideFromDocs: true` on `@All` routes - they are excluded automatically.
</Aside>
...
@Transient()
export class DocsFilterMiddleware implements Middleware {
  constructor(
...
See [Interactive Docs UI](/openapi/docs-ui/) to learn about the interactive documentation viewer and how to customize it.
